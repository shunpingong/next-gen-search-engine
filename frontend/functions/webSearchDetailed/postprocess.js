...
async function postprocess(llmOutput, environment) {
  console.log("=== Postprocessing Detailed Web Search ===");
  ...
  if (!llmOutput || typeof llmOutput !== "string") {
    return {
      topKSummary: "",
      error: "No valid output from LLM.",
    };
  }
  ...
  environment.temporaryStorage.webSearchDetailed ??= {};
  environment.temporaryStorage.webSearchDetailed.finalSummary = cleaned;

  console.log("Final cleaned & sanitized summary:");
  console.log(cleaned);
